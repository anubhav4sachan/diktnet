...
class ElmoPtrBaseDSTC(PtrBase):
    def __init__(self, config, corpus):
        super(ElmoPtrBaseDSTC, self).__init__(config)
...
###############################
############################## DSTC Based LAPtrHRED
##############################
class LAPtrHRED(ElmoPtrBaseDSTC):
    def valid_loss(self, loss, batch_cnt=None):
    ...
    def forward(self, data_feed, mode, gen_type='greedy', return_latent=False):
        ...
        if list(utt_outs.size())[1] != list(ctx_outs.size())[1]:
            if list(utt_outs.size())[1] < list(ctx_outs.size())[1]:
                i_s = list(ctx_outs.size())[0]
                j_s = list(ctx_outs.size())[1]
                k_s = list(ctx_outs.size())[2]
                j_final = list(ctx_outs.size())[1] - list(utt_outs.size())[1]
                zero_tens = torch.zeros([i_s, j_final, k_s], dtype=torch.float, device=torch.device('cuda:0'))
                sampl = torch.cat((utt_outs, zero_tens), 1)
