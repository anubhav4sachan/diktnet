...
from laed.modules import sent_models, dialog_models
...
def load_laed_features(in_folder):
    ...
    for dataset in ['train', 'valid', 'test']:
        with open(os.path.join(in_folder, 'dstc_dialogs_{}.pkl'.format(dataset)), 'rb') as laed_z_dialog_in:
...
class LAEDTuneCorpus(object):
    ...
    def _read_file(self, path):
        ...
        for filename in os.listdir(path):
            with open(os.path.join(path, filename)) as domain_in:
...
class DSTCZslCorpus(object):
    ...
    def __init__(self, config):
        ...
        self.black_domains = config.black_domains
        self.black_ratio = config.black_ratio
        self.speaker_map = {'SYSTEM': SYS, 'USER': USR}
        self.domain_descriptions = []

        self._load_laed_z()

        self.train_corpus = self._read_file_with_laed_z(os.path.join(self._path, 'train'),
                                                        self.laed_z['train'])
        self.valid_corpus = self._read_file_with_laed_z(os.path.join(self._path, 'dev'),
                                                        self.laed_z['valid'])
        self.test_corpus = self._read_file_with_laed_z(os.path.join(self._path, 'test'),
                                                       self.laed_z['test'])
        self.domains = set()
        for item in self.train_corpus:
            for subitem in item:
                for i in subitem['domain']:
                    self.domains.add(i)
        # self.domains = set([dialog[0].domain for dialog in self.train_corpus])

        with open(os.path.join(self._path, 'train/schema.json'), 'rb') as f:
            schema_ent = json.load(f)
            service_name = set()
            slots_names = set()
            is_categorical = set()
            slot_possible_values = set()
            
            intent_name = set()
            is_transactional = set()
            required_slots_values = set()
            
            for item in schema_ent:
                service_name.add(item['service_name'])
                
                for subitem in item['slots']:
                    slots_names.add(subitem['name'])
                    is_categorical.add(subitem['is_categorical'])
                    for values in subitem['possible_values']:
                        slot_possible_values.add(values)
                        
                for subitem in item['intents']:
                    intent_name.add(subitem['name'])
                    is_transactional.add(subitem['is_transactional'])
                    for values in subitem['required_slots']:
                        required_slots_values.add(values)
                        
                        
            slots_names = list(slots_names)
            is_categorical = list(is_categorical)
            slot_possible_values = list(slot_possible_values)
            
            intent_name = list(intent_name)
            is_transactional = list(is_transactional)
            required_slots_values = list(required_slots_values)

            ent_dict = {
                'service_name': service_name,
                'slots_name': slots_names,
                'is_categorical': is_categorical,
                'slot_possible_values': slot_possible_values,
                'intent_name': intent_name,
                'is_transactional': is_transactional,
                'required_slots_values': required_slots_values
            }
            ent_dict_list = []
            ent_dict_list.append(ent_dict)
            
            self.ent_metas = ent_dict
        #     self.ent_metas = json.load(f)
            # if self.config.lowercase:
            #     self.ent_metas = self._lowercase_json(self.ent_metas)
    ...
    def _read_file_with_laed_z(self, path, in_laed_z):
        ...
        for filename in os.listdir(path):
            if filename != 'schema.json':
                with open(os.path.join(path,filename),'rb') as domain_in:
                    data.append(json.load(domain_in))
                    # if self.config.lowercase:
                    #     data = self._lowercase_json(data)
            else:
                with open(os.path.join(path,filename),'rb') as domain_in:
                    schema.append(json.load(domain_in))
                    # if self.config.lowercase:
                    #     schema = self._lowercase_json(schema)
        # with open(path, 'rb') as f:
        #     data = json.load(f)
        #     if self.config.lowercase:
        #         data = self._lowercase_json(data)
        return self._process_dialog(data, in_laed_z, schema)

    def _process_dialog(self, data, laed_z_data, schema):
        new_dialog = []
        all_lens = []
        all_dialog_lens = []
        for r_dialog, laed_z in zip(data, laed_z_data):
            for raw_dialog in r_dialog:
                domain = raw_dialog['services']
                kb_items = []
                for schema_dict in schema[0]:
                    for dom in domain:
                        if schema_dict['slots'] is not None:
                            if schema_dict['service_name'] == dom:
                                for k,v in schema_dict['slots'][0].items():
                                    kb_items.append([KB] + self.tokenize("{} {}".format(k, v)))
                # if raw_dialog['scenario']['kb']['items'] is not None:
                ...
                dialog = [Pack(utt=[BOS, domain, BOD, EOS], speaker=USR, slots=None, domain=domain)]
                for turn, laed_z_turn in zip(raw_dialog['turns'], laed_z):
                    utt = turn['utterance']
                    slots = turn['frames'][0]['slots']
                    # utt = turn['data']['utterance']
                    # slots = turn['data'].get('slots')
                    speaker = self.speaker_map[turn['speaker']]
                    if self.config.include_domain:
                        utt = [BOS, speaker, domain] + self.tokenize(utt) + [EOS]
                    else:
                        utt = [BOS, speaker] + self.tokenize(utt) + [EOS]
    ...
    def _process_domain_description(self, in_turn, in_domain, in_laed_z):
        utt = in_turn['utterance']
        speaker = self.speaker_map[in_turn['speaker']]
    ...
    def _build_vocab(self):
        ...
        nall_words = []
        for item_all_words in all_words:
            if type(item_all_words) is list:
                for item in item_all_words:
                    nall_words.append(item)
            else:
                nall_words.append(item_all_words)
        nall_words_str = " ".join(nall_words)
        vocab_count = Counter(nall_words).most_common()
        raw_vocab_size = len(vocab_count)
        discard_wc = np.sum([c for t, c, in vocab_count])
        ...
        print("Load corpus with train size %d, valid size %d, "
              ...
              % (len(self.train_corpus), len(self.valid_corpus),
                 len(self.test_corpus),
                 raw_vocab_size, len(vocab_count), vocab_count[-1][1],
                 float(discard_wc) / len(all_words))) #vocab_count[-1][1],
    ...
    def _to_id_corpus(self, name, data, laed_z, use_black_list, domains):
        ...
        for dialog, dialog_laed_z in zip(data, laed_z):
            ...
            # convert utterance and feature into numeric numbers
            for turn, laed_z_turn in zip(dialog, dialog_laed_z):
                id_turn = Pack(utt=turn.utt,     #self._sent2id(turn.utt)
                               utt_raw=turn.utt,
                               speaker=turn.speaker,
                               domain=turn.domain,       #domain_id=self.rev_vocab[domain[0]],
                               meta=turn.get('meta'),
                               kb=[self._sent2id(item) for item in turn.get('kb', [])],
                               kb_raw=turn.get('kb', []),
                               laed_z=laed_z_turn)
            ...
            domain_cnt.append(domain)
        self.logger.info("Filter {} samples from {}".format(kick_cnt, name))
        self.logger.info(len(domain_cnt))
        # self.logger.info(Counter(domain_cnt).most_common())
        return results
...
# class DSTCGencorpus(object):
#     logger = logging.getLogger()

#     def __init__(self, config):
#         self.config = config
#         self._path = config.data_dir[0]
#         # self._path = dstc8-schema-guided-dialogue-master
#         self.max_utt_len = config.max_utt_len
#         self.tokenize = get_tokenize()
#         self.domain_l=[]
#         self.domain_present={}
#         self.train_corpus = self._read_file(os.path.join(self._path,'train'))
#         self.test_corpus  = self._read_file_t(os.path.join(self._path,'train'))
#         self.valid_corpus = self._read_file(os.path.join(self._path,'dev'))
#         """self.train_corpus, devtest = train_test_split(self.corpus,
#                                                       test_size=0.2,
#                                                       random_state=271)
#         self.valid_corpus, self.test_corpus = train_test_split(devtest,
...
#         return self._process_dialogs_t(data, exclude_domains=self.config.exclude_domains)




#     def _process_dialogs_t(self, data, exclude_domains=[]):
#         new_dialog = []
#         bod_utt = [BOS, BOD, EOS]
#         eod_utt = [BOS, EOD, EOS]
#         all_lens = []
#         exclude_domains=['Homes_1','Restaurants_1']
#         all_dialog_lens = []
#         speaker_map = {'bot': SYS, 'user': USR}
#         # file_count=0
#         for count in data:
...
#             # if file_count>43:
#                 # break
#             for raw_dialog in count:
#                 # print(raw_dialog)
#                 # priiiiit()
#                 # print("")
#                 if raw_dialog.get('services') is None:
#                     # print(raw_dialog)
#                     # priiiit()
#                     continue
#                 check=False
#                 # print("Harsh")
#                 # priit()
#                 for domain in raw_dialog['services']:
#                     if(self.domain_present.get(domain) is None):
#                         self.domain_l.extend(self.tokenize(domain))
#                         self.domain_present[domain]=1

#                     # domain = raw_dialog['services']
#                     # if domain in exclude_domains:
#                         # continue
#                 if len(raw_dialog['services'])>1:
#                     continue

#                 for domain in raw_dialog['services']:
#                     # if(self.domain_present.get(domain) is None):
#                         # self.domain_list=self.domain_list+self.tokenize(domain)
#                         # self.domain_present[domain]=1
...
#                     author_type = 'bot' if  turn['speaker'] == 'SYSTEM' else 'user'
#                     if self.config.include_domain:
#                         utt = [BOS, speaker_map[author_type]]
#                         for domain in raw_dialog['services']:
#                             utt = utt + [domain]
#                         utt = utt + self.tokenize(turn['utterance']) + [EOS]
#                     else:
#                         utt = [BOS, speaker_map[author_type]] + self.tokenize(turn['utterance']) + [EOS]
#                     all_lens.append(len(utt))
#                     dialog.append(Pack(utt=utt, speaker=speaker_map[author_type]))
...
#         vocab_count = Counter(all_words).most_common()
#         domain_count = Counter(self.domain_l).most_common()
#         raw_vocab_size = len(vocab_count)
#         discard_wc = np.sum([c for t, c, in vocab_count[max_vocab_cnt:]])
#         vocab_count = vocab_count[0: max_vocab_cnt]
...
#         self.vocab = [PAD, UNK, SYS, USR] + [t for t, cnt in vocab_count] + [t1 for t1,cnt1 in domain_count]
#         self.rev_vocab = {t: idx for idx, t in enumerate(self.vocab)}
#         self.unk_id = self.rev_vocab[UNK]
